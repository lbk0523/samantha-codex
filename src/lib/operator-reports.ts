...
export function remoteHelpReport(mode: "basic" | "advanced" = "basic"): string {
  if (mode === "advanced") {
    return [
      "# remote:help",
      "",
      "Telegram 고급 명령 목록은 dogfood 표면에서 제거했습니다.",
      "",
      "지금은 명령을 고르는 대신 Samantha에게 자연어로 말하세요.",
      "예: `Samantha Phase 1 상태와 다음 안전 행동을 짧게 보고해줘. 쓰기 작업 없이.`",
      "",
      `도움말 다시 보기: ${code("/help")}`,
    ].join("\n");
  }
  ...
  return [
    "# remote:help",
    "",
    "Samantha에게 자연어로 말하세요.",
    "",
    "예시:",
    "- `지금 Samantha Phase 1 상태와 남은 blocker를 짧게 보고해줘.`",
    "- `다음 report-only dogfood에서 확인할 evidence를 정리해줘. 파일 수정 없이.`",
    "- `이 계획은 승인해. 단, 실행은 deterministic gate를 따라가.`",
    "",
    "원칙:",
    "- report-only 요청은 가능한 범위까지 Samantha가 진행하고 결과나 blocker를 보고합니다.",
    "- write, recovery, merge, push, cleanup은 deterministic gate와 명시 승인 없이는 실행하지 않습니다.",
    "- 프로젝트가 애매하면 Samantha가 먼저 어느 프로젝트인지 묻습니다.",
    "",
    `도움말 다시 보기: ${code("/help")}`,
  ].join("\n");
}
