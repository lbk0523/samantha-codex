...
| --- | --- | --- |
| BK can inspect the state of work from one operating surface. | Met. CLI, dashboard, compact remote notifications, and `/now` share deterministic CEO status and next-action wording without requiring routine raw state-file inspection. | [docs/ARCHITECTURE.md](ARCHITECTURE.md), [docs/SAMANTHA_WORKFLOW_PLAYBOOK.md](SAMANTHA_WORKFLOW_PLAYBOOK.md), [tests/operating-surface.test.ts](../tests/operating-surface.test.ts), [tests/dashboard.test.ts](../tests/dashboard.test.ts), [tests/ceo-status.test.ts](../tests/ceo-status.test.ts) |
