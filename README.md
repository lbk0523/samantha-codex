...
Telegram can prepare and approve one safe dispatch action at a time with `/prepare-dispatch <task-id>` followed by `/approve-action <action-id>`. Approval does not run inside `inbox:watch`; `actions:watch` picks up approved actions and runs the prebuilt `tasks:dispatch <task-id> --allocate --execute --tmux` shape using the locally configured `SAMANTHA_REPO_ROOT`. Telegram input cannot supply shell commands, repo paths, merge, push, or cleanup actions.
