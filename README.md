...
Telegram's primary operating flow is `/work <request>`, `/plan`, then `/go`. `/plan` prepares the latest draft from project profile defaults and remote scope recipes, then returns the execution plan, target scope, verification, and safety gates. `/go` approves the ready draft or pending action and queues the prebuilt `tasks:dispatch <task-id> --allocate --execute --tmux` action. Approval does not run inside `inbox:watch`; `actions:watch` picks up approved actions using the task project repo root or locally configured `SAMANTHA_REPO_ROOT`. Telegram input cannot supply shell commands, merge, push, cleanup actions, or arbitrary repo paths. Use `/help_advanced` for the explicit inspection and action-id commands.
