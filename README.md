...
Telegram's primary operating flow is `/work <request>`, `/draft_next`, `/draft_prepare <project_id> [target_file...]`, `/draft_approve`, `/run_next`, then `/yes`. `/draft_prepare` applies project defaults and repo-relative target files to the latest draft; `/draft_approve` promotes a ready draft to a pending task without dispatching a worker. `/run_next` prepares the pending task as a safe dispatch action, and `/yes` approves the latest prepared action. Approval does not run inside `inbox:watch`; `actions:watch` picks up approved actions and runs the prebuilt `tasks:dispatch <task-id> --allocate --execute --tmux` shape using the task project repo root or locally configured `SAMANTHA_REPO_ROOT`. Telegram input cannot supply shell commands, merge, push, cleanup actions, or arbitrary repo paths. Use `/help_advanced` for the explicit inspection and action-id commands.
