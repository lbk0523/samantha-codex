...
Samantha-Codex는 BK가 하나의 orchestrator surface와 대화하면, 결정적인 TypeScript 코드가 작업 상태와 safety gate를 관리하고, 필요한 순간에만 제한된 Codex agent를 호출해 planning, execution, review, evaluation, reporting을 수행하는 개인 운영 레이어입니다.
...
```text
BK
  <-> local operator CLI / dashboard / Telegram approval adapter
...
CEO office는 상태, 실행 권한, 안전을 소유합니다. LLM agent는 제안, 분석, 리뷰, 보고서 초안을 담당하며, 상태를 기억하기 위해 상주하지 않고 deterministic gate를 우회하지 않습니다.
...
Telegram command surface는 의도적으로 작게 유지하며, 핵심 제품 표면이 아니라 notification/approval adapter로 취급합니다. 현재 존재하는 원격 흐름은 다음과 같습니다.
